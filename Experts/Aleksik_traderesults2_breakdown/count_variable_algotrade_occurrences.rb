...
ENTRY_FORGET_MIN_ROOM_MINUTES = 30
...
def entry_forget_reachability(entry_max_min, forget_candles)
  ...
  valid_with_room = room_min >= ENTRY_FORGET_MIN_ROOM_MINUTES
  {
...
if output_rows.empty?
  ...
  output_rows.each do |row|
    puts "  #{COUNT_VARIABLE}=#{row['variable_value']}: " \
...
if forget_by_algo_id.empty?
  ...
  puts "entry/forget reachability (room rule: forget_min - entry_max >= #{ENTRY_FORGET_MIN_ROOM_MINUTES}):"
  ...
  invalid_algos =
    config_by_algo_id.filter_map do |algo_id, config_row|
      ...
      reach = entry_forget_reachability(entry_max, forget_candles)
      reach[:valid_with_room] ? nil : algo_id
    end
