...
trade_groups.each do |trade_id, rows|
  ...
  above.combination(2).each do |c|
    setup_dict[[magic,"price-below",c[0][:ref],c[1][:ref]]] << [c[0][:filters], profit]
  end

  below.combination(2).each do |c|
    setup_dict[[magic,"price-above",c[0][:ref],c[1][:ref]]] << [c[0][:filters], profit]
  end

  above.product(below).each do |c|
    setup_dict[[magic,"price-betweenH-L",c[0][:ref],c[1][:ref]]] << [c[0][:filters], profit]
  end
...
by_magic.each do |magic, rows|
  ...
  edges = rows.map do |r|
    {factor: r[1], count: r[2], winrate: r[3], avg: r[4]}
  end

  top =
    edges
    .select { |e| e[:winrate] > 0.7 }
    .sort_by { |e| -e[:winrate] }
    .first(3)

  bottom =
    edges
    .select { |e| e[:winrate] < 0.3 }
    .sort_by { |e| e[:winrate] }
    .first(3)
