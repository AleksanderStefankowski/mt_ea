...
# Bulk-edit g_levelAlgos fields in aleksik2_level_fam.mqh for algos whose FILTER_VARIABLE matches FILTER_VALUES.
...
ALLOWED_FIELDS = %w[
  ...
  cannotTrade__when_thisAlgoOpenOrPendingNearLevel
  real_tp
  rule_switch_map
].freeze
...
PARAM_ASSIGN_RE = /
  g_levelAlgos\[LevelAlgoSlotIndexByAlgoId\(MAGIC_LEVEL(\d+)\)\]\.(\w+)\s*=\s*([^;]+);
/x

module LevelEditByVariable
  module_function
  ...
  def level_params_by_algo_id(content)
    inner = LevelCombinationsCreator.extract_inner(content, LEVEL_FAM_MARKERS[2])
    params = Hash.new { |hash, key| hash[key] = {} }
    inner.scan(PARAM_ASSIGN_RE) do |id, field, value|
      params[id.to_i][field] = strip_mq5_value(value)
    end
    params
  end

  def normalize_filter_value(_field, raw)
    LevelCombinationsCreator.normalize_combo_value(raw)
  end
  ...
  def matching_algo_ids(content, field, values)
    params = level_params_by_algo_id(content)
  ...
  def format_mq5_field_value(field, value)
    case field
    when *DOUBLE_FIELDS
      decimals = field == "offset_percentage" ? 4 : 2
      LevelCombinationsCreator.format_mq5_double(value, decimals)
    when *BOOL_FIELDS
      bool =
        case value
      ...
      LevelCombinationsCreator.format_mq5_bool(bool)
    else
      s = value.to_s.strip
  ...
  def apply_edits!(content, targets_by_algo)
    ...
    updated = content.gsub(PARAM_ASSIGN_RE) do |match|
      ...
      current_raw = strip_mq5_value(Regexp.last_match(3))
...
if __FILE__ == $PROGRAM_NAME
  ...
  mq5_content = File.read(MQ5_FILE)
  level_fam_content = File.read(LEVEL_FAM_FILE)
  params_by_algo = level_params_by_algo_id(level_fam_content)
  wired_ids = LevelCombinationsCreator.registry_algo_ids(mq5_content)
  match_ids = matching_algo_ids(level_fam_content, FILTER_VARIABLE, FILTER_VALUES)
  ...
  puts "Level fam file:           #{LEVEL_FAM_FILE}"
  ...
  if match_ids.empty?
    puts "No level algos match the filter."
    exit 0
  end

  _updated, changed_by_field, skipped_by_field, missing_by_field = apply_edits!(level_fam_content, targets_by_algo)
  ...
  print "Apply edits to #{targets_by_algo.size} level algo(s)? [y/N] "
  ...
  updated, changed_by_field, skipped_by_field, = apply_edits!(level_fam_content, targets_by_algo)
  File.write(LEVEL_FAM_FILE, updated)
