...
CHECK_MINIMUM_TRADERATEWEEKLY_VALUE = 0.41
...
slice_summaries.each do |summary|
  ...
  best = summary[:best]
  puts format(
    '%s | min_trades=%d | pf=%.2f | grp_trades=%d | traderate=%.2f | weekly_traderate=%.2f',
    summary[:set_name],
    summary[:threshold],
    best[:pf],
    best[:trades],
    best[:group_trade_rate],
    best[:group_weekly_trade_rate]
  )
end
