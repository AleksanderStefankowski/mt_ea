...
# Edits the four //algocreator* blocks.
...
add_to_weekly_exception_list = false

def run_independent!(tune_overrides_text:, rules_text:, add_to_weekly_exception_list:)
  content = read_mq5
  ...
  new_b3 = add_to_weekly_exception_list ? update_block3_add(b3, new_id) : b3
  ...
  puts "Weekly exception list: #{add_to_weekly_exception_list ? 'yes' : 'no'}"
  puts
...
if __FILE__ == $PROGRAM_NAME
  run_independent!(
    ...
    rules_text: rules,
    add_to_weekly_exception_list: add_to_weekly_exception_list
  )
end
