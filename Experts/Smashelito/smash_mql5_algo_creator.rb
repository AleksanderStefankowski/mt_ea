...
# appending subvariant-style extra rules. Edits the four //algocreator* blocks.
...
# One rule per line (subvariant-style). Leave empty for none.
# Shorthand below_PDH=true also accepts below_PDH on its own.
extra_rules = ''
...
module AlgoCreator
  ...
  def mql5_line_for_extra_rule(token)
    ...
    raise "Unknown extra rule token: #{token.inspect}" unless call
  ...
  def clone_rule_case(case_text, source_id, new_id, extra_tokens)
    ...
    # Single-algo case only for the new block (no shared case labels).
    body_start = lines.index { |l| l.match?(/^\s*case\s+/) }
    break_idx = lines.index { |l| l.match?(/^\s*break\s*;/) }
    body = lines[(body_start + 1)...break_idx]

    existing_calls = body.map(&:strip)
    extra_lines = []
    extra_tokens.each do |token|
      line = mql5_line_for_extra_rule(token)
      extra_lines << line unless existing_calls.include?(line.strip)
    end

    insert_before = body.index { |l| l.include?('RULE_TRADES_AT_LEVEL_LIMIT') } || body.length
    new_body = body[0...insert_before] + extra_lines + body[insert_before..]
  ...
  def run(copy_from:, extra_rules_text:)
    ...
    puts "Extra rules applied: #{extra_tokens.empty? ? '(none)' : extra_tokens.join(', ')}"
    puts
